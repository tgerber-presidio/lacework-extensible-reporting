...
def compliance_reports_summary_for_graphic_gcp(compliance_reports, severities=["Critical", "High", "Medium", "Low"]):
    ...
    # sort to determine account order, while we still have the severity & resource count data
    df = df.sort_values(by=['SEVERITY', 'RESOURCE_COUNT'],ascending=[True,False])
    account_order = df['PROJECT_ID'].unique()
    
    # group
    df = df.groupby(['PROJECT_ID', 'SEVERITY']).agg(failed_control_count=('SEVERITY', 'count'), failed_resources=('RESOURCE_COUNT', 'sum')).reset_index()
    
    # sort by accounts with most criticals, followed by most highs
    df['PROJECT_ID'] = pd.Categorical(df['PROJECT_ID'], account_order)
    df.sort_values(by=['PROJECT_ID', 'SEVERITY'], ascending=True)
    ...
    # rename    
    df.rename(columns={'PROJECT_ID': 'Project ID', 'SEVERITY': 'Severity', 'failed_resources': 'Non-compliant Resources', 'failed_control_count': 'Failed controls'}, inplace=True)
    

    # pivot and preseve severity order (pivot breaks this)
    severity_order = df['Severity'].unique()
    df = pd.pivot_table(df, values='Non-compliant Resources', index='Project ID', columns='Severity', sort=False)
    df = df.reindex(severity_order, axis=1)
