...
def generate_report(_shared, report_save_path, use_cached_data):
    ...
    gcp_compliance_data = gather_compliance_data_gcp(_shared, lw_provider)
...
def gather_compliance_data_gcp(_shared, lw_provider):
    ...
    # get ogranizations
    org_ids = _shared.t_lw.integrations_config_accounts_gcp(integrations)
    if not org_ids:
        return False

    # get compliance reports
    gcp_compliance_reports = lw_provider.compliance_reports_gcp(org_ids=org_ids,lw_provider=lw_provider)
    gcp_compliance_reports = _shared.t_lw.compliance_reports_select_most_noncompliant(gcp_compliance_reports)

    #flatten
    gcp_compliance_reports = sum(map(lambda kv: kv[1], gcp_compliance_reports.items()), [])

    # set table classes
    compliance_detail = _shared.t_lw.compliance_reports_raw_gcp(gcp_compliance_reports)
    compliance_detail = compliance_detail.style.set_table_attributes('class="compliance_detail"')
    compliance_summary = _shared.t_lw.compliance_reports_summary_gcp(gcp_compliance_reports)
    compliance_summary = compliance_summary.style.set_table_attributes('class="compliance_summary"')

    # get graphics
    compliance_findings_summary_for_graphic = _shared.t_lw.compliance_reports_summary_for_graphic_gcp(gcp_compliance_reports)

    compliance_findings_by_account_bar_graphic = _shared.g_lw_plotly.compliance_findings_summary_by_account_bar(compliance_findings_summary_for_graphic, width=1200)
    compliance_findings_by_account_bar_graphic = _shared.common.bytes_to_image_tag(compliance_findings_by_account_bar_graphic, 'svg+xml')
    
    compliance_reports_summary_by_service_for_graphic = _shared.t_lw.compliance_reports_summary_by_service_for_graphic_gcp(gcp_compliance_reports)
